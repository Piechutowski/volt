...
| PERF-10 | Per-declaration memoization for the one-file layout (D83, D84): a file is parsed in chunks, one per top-level declaration, an edit re-parses the chunk it touched and relocates the rest; the package's tables, models, lowered checks and selects are answered from the last check when their inputs are the objects they were. Thousand-table file, one edit: parse 340 ms to 22 ms, check 700 ms to about 90 to 190 ms. Route expansion still runs whole, about 50 ms of what remains. The server's navigation index is kept by table as well (D85): its whole analysis of that edit is 106 ms best, 163 ms mean, against 400 to 650. Every memoized computation is a pure function of explicit inputs, proven by the purity gate (D86); every memo key is the inputs themselves, nothing hashed, no size or time stamp (D87) | `DONE` |
