...
| `corpus/gostd/` | `go doc -all`, installed Go toolchain (go1.26.5) | Plain text |
...
The Go corpus is different in kind: it is the *baseline*, not a
competitor. Go's documentation lives in doc comments, so
`fetch_gostd.sh` dumps `go doc -all` for the ~105 web-relevant standard
library packages (including the experiment-gated `encoding/json/v2`)
from whatever Go toolchain is on PATH.
...
Requirements: `git`, `pandoc` (Django only), `go` (gostd only).
Version resolution is automatic — highest stable branch/tag via
`git ls-remote` — and each script accepts an env override
(`LARAVEL_REF`, `RAILS_REF`, `PHOENIX_REF`, `LIVEVIEW_REF`, `ECTO_REF`,
`PLUG_REF`, `DJANGO_REF`). The Go corpus is pinned by the installed
toolchain: `./fetch_gostd.sh` re-dumps from whatever `go version` says.
...
`django.md` — plus `gostd.md` and `comparison.md`. All follow the same
skeleton (P1 routing … P21 admin, shared ID prefixes like `ROUTE-`,
`ORM-`, `AUTH-`), in the style of not-an-orm's `features.md`:
problem-oriented sections, one terse row per feature with a stable ID,
a tier marker, and source file references back into the corpus. The
shared skeleton is what makes the comparison matrix line up
row-for-row.

`gostd.md` is the baseline document: the same skeleton applied to the
Go standard library (tiers `STD` / `TOOL` / `X` / `NO`), where the `NO`
rows are the point — they mark what Volt has to provide. Its closing
section, "The gap Volt fills", is the short version. `comparison.md`
carries **Go std** as the first column of every capability matrix, so
each row reads as "what Go gives for free vs how the four frameworks
answer it"; its "What Go already covers" section turns the table-stakes
list into a build-vs-free map for Volt.
