...
<div class="module" synopsis="Django's signing framework.">
...
See `signing-complex-data` for more details.
...
<div class="Signer(*, key=None, sep=':', salt=None, algorithm=None, fallback_keys=None)">
...
<4648#section-5>`. This alphabet contains alphanumeric characters, hyphens, and underscores. `algorithm` must be an algorithm supported by `hashlib`, it defaults to `'sha256'`. `fallback_keys` is a list of additional values used to validate signed data, defaults to `SECRET_KEY_FALLBACKS`.
...
<div class="TimestampSigner(*, key=None, sep=':', salt=None, algorithm='sha256')">
