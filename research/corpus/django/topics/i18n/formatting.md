...
Django's formatting system is capable of displaying dates, times and numbers in templates using the format specified for the current `locale <locale name>`. It also handles localized input in forms.
...
To enable number formatting with thousand separators, it is necessary to set `USE_THOUSAND_SEPARATOR = True <USE_THOUSAND_SEPARATOR>` in your settings file. Alternatively, you could use `intcomma` to format numbers in your template.
...
There is a related `USE_I18N` setting that controls if Django should activate translation. See `/topics/i18n/translation` for more details.
...
For country specific form fields, validation, and data formats that are not included in Django itself, consider using the `django-localflavor` package, which provides localized form fields and utilities for many countries and regions ([documentation](https://django-localflavor.readthedocs.io/)).
...
When localization is disabled, the `localization settings <settings-l10n>` formats are applied.
...
Returns a string representation for numbers (`int`, `float`, or `Decimal`) with the `localization settings <settings-l10n>` formats applied.
