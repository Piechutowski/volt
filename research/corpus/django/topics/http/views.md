...
- First, we import the class `~django.http.HttpResponse` from the `django.http` module, along with Python's `datetime` library.

- Next, we define a function called `current_datetime`. This is the view function. Each view function takes an `~django.http.HttpRequest` object as its first parameter, which is typically named `request`.
...
- The view returns an `~django.http.HttpResponse` object that contains the generated response. Each view function is responsible for returning an `~django.http.HttpResponse` object. (There are exceptions, but we'll get to those later.)
...
So, to recap, this view function returns an HTML page that includes the current date and time. To display this view at a particular URL, you'll need to create a *URLconf*; see `/topics/http/urls` for instructions.

## Returning errors

Django provides help for returning HTTP error codes. There are subclasses of `~django.http.HttpResponse` for a number of common HTTP status codes other than 200 (which means *"OK"*). You can find the full list of available subclasses in the `request/response <ref-httpresponse-subclasses>` documentation. Return an instance of one of those subclasses instead of a normal `~django.http.HttpResponse` in order to signify an error. For example:
...
There isn't a specialized subclass for every possible HTTP response code, since many of them aren't going to be that common. However, as documented in the `~django.http.HttpResponse` documentation, you can also pass the HTTP status code into the constructor for `~django.http.HttpResponse` to create a return class for any status code you like. For example:
...
<div class="django.http.Http404()">

When you return an error such as `~django.http.HttpResponseNotFound`, you're responsible for defining the HTML of the resulting error page:
...
The `~django.views.defaults.page_not_found` view is overridden by `~django.conf.urls.handler404`:

    handler404 = "mysite.views.my_custom_page_not_found_view"

The `~django.views.defaults.server_error` view is overridden by `~django.conf.urls.handler500`:

    handler500 = "mysite.views.my_custom_error_view"

The `~django.views.defaults.permission_denied` view is overridden by `~django.conf.urls.handler403`:

    handler403 = "mysite.views.my_custom_permission_denied_view"

The `~django.views.defaults.bad_request` view is overridden by `~django.conf.urls.handler400`:
...
You can read more about Django's async support, and how to best use async views, in `/topics/async`.
