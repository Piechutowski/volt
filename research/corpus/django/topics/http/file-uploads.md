...
When Django handles a file upload, the file data ends up placed in `request.FILES <django.http.HttpRequest.FILES>` (for more on the `request` object see the documentation for `request and response objects
...
Consider a form containing a `~django.forms.FileField`:
...
A view handling this form will receive the file data in `request.FILES <django.http.HttpRequest.FILES>`, which is a dictionary containing a key for each `~django.forms.FileField` (or `~django.forms.ImageField`, or other `~django.forms.FileField` subclass) in the form. So the data from the above form would be accessible as `request.FILES['file']`.

Note that `request.FILES <django.http.HttpRequest.FILES>` will only contain data if the request method was `POST`, at least one file field was actually posted, and the `<form>` that posted the request has the attribute `enctype="multipart/form-data"`. Otherwise, `request.FILES` will be empty.

Most of the time, you'll pass the file data from `request` into the form as described in `binding-uploaded-files`. This would look something like:
...
<django.http.HttpRequest.FILES>` into the form's constructor; this is how file data gets bound into a form.
...
If you're saving a file on a `~django.db.models.Model` with a `~django.db.models.FileField`, using a `~django.forms.ModelForm` makes this process much easier. The file object will be saved to the location specified by the `~django.db.models.FileField.upload_to` argument of the corresponding `~django.db.models.FileField` when calling `form.save()`:
...
If you are constructing an object manually, you can assign the file object from `request.FILES <django.http.HttpRequest.FILES>` to the file field in the model:
...
If you are constructing an object manually outside of a request, you can assign a `~django.core.files.File` like object to the `~django.db.models.FileField`:
...
Then override the `form_valid()` method of your `~django.views.generic.edit.FormView` subclass to handle multiple file uploads:
...
There are a few settings which control Django's file upload behavior. See `File Upload Settings <file-upload-settings>` for details.
...
Also, `request.POST` is accessed by `~django.middleware.csrf.CsrfViewMiddleware` which is enabled by default. This means you will need to use `~django.views.decorators.csrf.csrf_exempt` on your view to allow you to change the upload handlers. You will then need to use `~django.views.decorators.csrf.csrf_protect` on the function that actually processes the request. Note that this means that the handlers may start receiving the file upload before the CSRF checks have been done. Example code:
...
If you are using a class-based view, you will need to use `~django.views.decorators.csrf.csrf_exempt` on its `~django.views.generic.base.View.dispatch` method and `~django.views.decorators.csrf.csrf_protect` on the method that actually processes the request. Example code:
