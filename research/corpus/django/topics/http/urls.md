...
1.  Django determines the root URLconf module to use. Ordinarily, this is the value of the `ROOT_URLCONF` setting, but if the incoming `HttpRequest` object has a `~django.http.HttpRequest.urlconf` attribute (set by middleware), its value will be used in place of the `ROOT_URLCONF` setting.
...
3.  Django runs through each URL pattern, in order, and stops at the first one that matches the requested URL, matching against `~django.http.HttpRequest.path_info`.
4.  Once one of the URL patterns matches, Django imports and calls the given view, which is a Python function (or a `class-based view
    </topics/class-based-views/index>`). The view gets passed the following arguments:
...
- `uuid` - Matches a formatted UUID. To prevent multiple URLs from mapping to the same page, dashes must be included and letters must be lowercase. For example, `075194d3-6885-417e-a8a8-6c931e272f00`. Returns a `~uuid.UUID` instance.
...
- A `to_url(self, value)` method, which handles converting the Python type into a string to be used in the URL. It should raise `ValueError` if it can't convert the given value. A `ValueError` is interpreted as no match and as a consequence `~django.urls.reverse` will raise `~django.urls.NoReverseMatch` unless another URL pattern matches.
...
Register custom converter classes in your URLconf using `~django.urls.register_converter`:
...
If the paths and converters syntax isn't sufficient for defining your URL patterns, you can also use regular expressions. To do so, use `~django.urls.re_path` instead of `~django.urls.path`.
...
When switching from using `~django.urls.path` to `~django.urls.re_path` or vice versa, it's particularly important to be aware that the type of the view arguments may change, and so you may need to adapt your views.
...
`urlpatterns` should be a `sequence` of `~django.urls.path` and/or `~django.urls.re_path` instances.
...
See the documentation on `customizing error views
...
Whenever Django encounters `~django.urls.include`, it chops off whatever part of the URL matched up to that point and sends the remaining string to the included URLconf for further processing.

Another possibility is to include additional URL patterns by using a list of `~django.urls.path` instances. For example, consider this URLconf:
...
The `~django.urls.path` function can take an optional third argument which should be a dictionary of extra keyword arguments to pass to the view function.
...
Similarly, you can pass extra options to `~django.urls.include` and each line in the included URLconf will be passed the extra options.
...
- In Python code: Using the `~django.urls.reverse` function.
- In higher level code related to handling of URLs of Django model instances: The `~django.db.models.Model.get_absolute_url` method.
...
When naming URL patterns, choose names that are unlikely to clash with other applications' choice of names. If you call your URL pattern `comment` and another application does the same thing, the URL that `~django.urls.reverse` finds depends on whichever pattern is last in your project's `urlpatterns` list.
...
You can deliberately choose the *same URL name* as another application if you want to override a view. For example, a common use case is to override the `~django.contrib.auth.views.LoginView`. Parts of Django and most third-party apps assume that this view has a URL pattern with the name `login`. If you have a custom login view and give its URL the name `login`, `~django.urls.reverse` will find your custom view as long as it's in `urlpatterns` after `django.contrib.auth.urls` is included (if that's included at all).

You may also use the same name for multiple URL patterns if they differ in their arguments. In addition to the URL name, `~django.urls.reverse` matches the number of arguments and the names of the keyword arguments. Path converters can also raise `ValueError` to indicate no match, see `registering-custom-path-converters` for details.
...
Django applications that make proper use of URL namespacing can be deployed more than once for a particular site. For example `django.contrib.admin` has an `~django.contrib.admin.AdminSite` class which allows you to `deploy more than one instance of the admin <multiple-admin-sites>`. In a later example, we'll discuss the idea of deploying the polls application from the tutorial in two different locations so we can serve the same functionality to two different audiences (authors and publishers).
...
2.  If there is a current application defined, Django finds and returns the URL resolver for that instance. The current application can be specified with the `current_app` argument to the `~django.urls.reverse` function.

    The `url` template tag uses the namespace of the currently resolved view as the current application in a `~django.template.RequestContext`. You can override this default by setting the current application on the `request.current_app
    <django.http.HttpRequest.current_app>` attribute.

3.  If there is no current application, Django looks for a default application instance. The default application instance is the instance that has an `instance namespace` matching the `application
...
Firstly, you can set an `app_name` attribute in the included URLconf module, at the same level as the `urlpatterns` attribute. You have to pass the actual module, or a string reference to the module, to `~django.urls.include`, not the list of `urlpatterns` itself.
...
Secondly, you can include an object that contains embedded namespace data. If you `include()` a list of `~django.urls.path` or `~django.urls.re_path` instances, the URLs contained in that object will be added to the global namespace. However, you can also `include()` a 2-tuple containing:
...
The instance namespace can be specified using the `namespace` argument to `~django.urls.include`. If the instance namespace is not specified, it will default to the included URLconf's application namespace. This means it will also be the default instance for that namespace.
