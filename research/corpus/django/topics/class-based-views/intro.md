...
In the beginning there was only the view function contract, Django passed your function an `~django.http.HttpRequest` and expected back an `~django.http.HttpResponse`. This was the extent of what Django provided.
...
Because Django's URL resolver expects to send the request and associated arguments to a callable function, not a class, class-based views have an `~django.views.generic.base.View.as_view` class method which returns a function that can be called when a request arrives for a URL matching the associated pattern. The function creates an instance of the class, calls `~django.views.generic.base.View.setup` to initialize its attributes, and then calls its `~django.views.generic.base.View.dispatch` method. `dispatch` looks at the request to determine whether it is a `GET`, `POST`, etc, and relays the request to a matching method if one is defined, or raises `~django.http.HttpResponseNotAllowed` if not:
...
Another option is to configure class attributes as keyword arguments to the `~django.views.generic.base.View.as_view` call in the URLconf:
...
While your class is instantiated for each request dispatched to it, class attributes set through the `~django.views.generic.base.View.as_view` entry point are configured only once at the time your URLs are imported.
...
For example, in the generic class-based views there is a mixin called `~django.views.generic.base.TemplateResponseMixin` whose primary purpose is to define the method `~django.views.generic.base.TemplateResponseMixin.render_to_response`. When combined with the behavior of the `~django.views.generic.base.View` base class, the result is a `~django.views.generic.base.TemplateView` class that will dispatch requests to the appropriate matching methods (a behavior defined in the `View` base class), and that has a `~django.views.generic.base.TemplateResponseMixin.render_to_response` method that uses a `~django.views.generic.base.TemplateResponseMixin.template_name` attribute to return a `~django.template.response.TemplateResponse` object (a behavior defined in the `TemplateResponseMixin`).
...
Note also that you can only inherit from one generic view - that is, only one parent class may inherit from `~django.views.generic.base.View` and the rest (if any) should be mixins. Trying to inherit from more than one class that inherits from `View` - for example, trying to use a form at the top of a list and combining `~django.views.generic.edit.ProcessFormView` and `~django.views.generic.list.ListView` - won't work as expected.
...
You can adjust class-based views by decorating the result of the `~django.views.generic.base.View.as_view` method. The easiest place to do this is in the URLconf where you deploy your view:
...
To decorate every instance of a class-based view, you need to decorate the class definition itself. To do this you apply the decorator to the `~django.views.generic.base.View.dispatch` method of the class.
...
In this example, every instance of `ProtectedView` will have login protection. These examples use `login_required`, however, the same behavior can be obtained by using `~django.contrib.auth.mixins.LoginRequiredMixin`.
