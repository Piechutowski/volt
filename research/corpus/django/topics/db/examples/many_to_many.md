...
Create and add a `Publication` to an `Article` in one step using `~django.db.models.fields.related.RelatedManager.create`:
...
The `~django.db.models.query.QuerySet.count` function respects `~django.db.models.query.QuerySet.distinct` as well:
...
Reverse m2m queries are supported (i.e., starting at the table that doesn't have a `~django.db.models.ManyToManyField`):
...
After the `~django.db.models.query.QuerySet.delete`, the `~django.db.models.query.QuerySet` cache needs to be cleared, and the referenced objects should be gone:
