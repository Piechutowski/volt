...
To define a one-to-one relationship, use `~django.db.models.OneToOneField`.
...
If you delete a place, its restaurant will be deleted (assuming that the `OneToOneField` was defined with `~django.db.models.ForeignKey.on_delete` set to `CASCADE`, which is the default):
