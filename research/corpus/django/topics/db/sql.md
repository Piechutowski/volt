...
Django gives you three ways of performing raw SQL queries: you can embed raw SQL fragments into ORM queries using `~django.db.models.expressions.RawSQL` (see `raw-sql-fragments`), use `Manager.raw` to [perform raw queries and return model instances](#performing-raw-queries), or avoid the model layer entirely and [execute custom SQL directly](#executing-custom-sql-directly).
...
- You can `annotate <.QuerySet.annotate>` and `aggregate
  </topics/db/aggregation>` using many built-in `database functions
  </ref/models/database-functions>`. Beyond those, you can create `custom query expressions </ref/models/expressions/>`.

Before using raw SQL, explore `the ORM </topics/db/index>`. Ask on one of `the support channels </faq/help>` to see if the ORM supports your use case.
...
In some cases, you may need to embed raw SQL fragments directly into ORM queries — for example, in `~django.db.models.query.QuerySet.annotate` or `~django.db.models.query.QuerySet.filter` calls. Use `Func()
expressions <func-expressions>` for calling database functions across backends, or `~django.db.models.expressions.RawSQL` for arbitrary parameterized SQL fragments.
...
This method takes a raw SQL query, executes it, and returns a `django.db.models.query.RawQuerySet` instance. This `RawQuerySet` instance can be iterated over like a normal `~django.db.models.query.QuerySet` to provide object instances.
...
For more details check out the documentation for the `~Options.db_table` option, which also lets you manually set the database table name.
...
The `Person` objects returned by this query will be deferred model instances (see `~django.db.models.query.QuerySet.defer`). This means that the fields that are omitted from the query will be loaded on demand. For example:
...
There is only one field that you can't leave out - the primary key field. Django uses the primary key to identify model instances, so it must always be included in a raw query. A `~django.core.exceptions.FieldDoesNotExist` exception will be raised if you forget to include the primary key.
...
If you are using `more than one database </topics/db/multi-db>`, you can use `django.db.connections` to obtain the connection (and cursor) for a specific database. `django.db.connections` is a dictionary-like object that allows you to retrieve a specific connection using its alias:
...
`connection` and `cursor` mostly implement the standard Python DB-API described in `249` — except when it comes to `transaction handling
