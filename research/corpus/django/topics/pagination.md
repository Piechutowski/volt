...
Under the hood, all methods of pagination use the `~django.core.paginator.Paginator` class. It does all the heavy lifting of actually splitting a `QuerySet` into `~django.core.paginator.Page` objects.

## Example

Give `~django.core.paginator.Paginator` a list of objects, plus the number of items you'd like to have on each page, and it gives you methods for accessing the items for each page:
...
`django.views.generic.list.ListView` provides a builtin way to paginate the displayed list. You can do this by adding a `~django.views.generic.list.MultipleObjectMixin.paginate_by` attribute to your view class, for example:
...
Here's an example using `~django.core.paginator.Paginator` in a view function to paginate a queryset:
