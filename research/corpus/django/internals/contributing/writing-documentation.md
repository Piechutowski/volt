...
We also backport documentation fixes and improvements, at the discretion of the merger, to the last release branch. This is because it's advantageous to have the docs for the last release be up-to-date and correct (see `differences-between-doc-versions`).

Django's documentation uses the [Sphinx](https://www.sphinx-doc.org/) documentation system, which in turn is based on [docutils](https://docutils.sourceforge.io/). The basic idea is that lightly-formatted plain-text documentation is transformed into HTML, PDF, and any other output format.
...
- `Tutorials </intro/index>` take the reader by the hand through a series of steps to create something.
...
- `Topic guides </topics/index>` aim to explain a concept or subject at a fairly high level.
...
- `Reference guides </ref/index>` contain technical references for APIs. They describe the functioning of Django's internal machinery and instruct in its use.
...
- `How-to guides </howto/index>` are recipes that take the reader through steps in key subjects.
...
If you'd like to start contributing to our docs, get the development version of Django from the source code repository (see `installing-development-version`):
...
These files are written in the reStructuredText markup language. To learn the markup, see the `reStructuredText reference <sphinx:rst-index>`.
...
Several checks help maintain Django's documentation quality, including `spelling <documentation-spelling-check>`, `code block formatting <documentation-code-block-format-check>`, and `documentation style <documentation-lint-check>`.
...
Before you commit your docs, it's a good idea to run the spelling checker. You'll need to install `sphinxcontrib-spelling` first. The spell checker also requires a system-level spell checking backend such as [Aspell](http://aspell.net/). Then from the `docs` directory, run:
...
All Python code blocks should be formatted using the `blacken-docs` auto-formatter. This is automatically run by the `pre-commit hook
...
- Use `~sphinx.ext.intersphinx` to reference Python's and Sphinx' documentation.
...
- Use `:rfc:<rfc>` to reference a Request for Comments (RFC) and try to link to the relevant section if possible. For example, use `` :rfc:`2324#section-2.3.2 ``<span class="title-ref"> or </span><span class="title-ref">:rfc:\`Custom link text \<2324#section-2.3.2\></span>\`\`.

- Use `:pep:<pep>` to reference a Python Enhancement Proposal (PEP) and try to link to the relevant section if possible. For example, use `` :pep:`20#easter-egg ``<span class="title-ref"> or </span><span class="title-ref">:pep:\`Easter Egg \<20#easter-egg\></span>\`\`.

- Use `:mimetype:<mimetype>` to refer to a MIME Type unless the value is quoted for a code example.

- Use `:envvar:<envvar>` to refer to an environment variable. You may also need to define a reference to the documentation for that environment variable using `.. envvar:: <envvar>`.

- Use `:cve:<cve>` to reference a Common Vulnerabilities and Exposures (CVE) identifier. For example, use `` :cve:`2019-14232 ``\`.

- When documenting Python objects (classes, methods, attributes, etc.) using [Sphinx directives](https://www.sphinx-doc.org/en/master/usage/restructuredtext/basics.html) such as `.. class::`, `.. method::`, and `.. attribute::`, all content must be properly indented to ensure correct rendering and to support features like automatic table of contents generation.
...
Besides `Sphinx's built-in markup <sphinx:rst-index>`, Django's docs define some extra description units:
...
- `django-admin` command-line options:
  ...
  To link, use `` :option:`command_name --traceback ``<span class="title-ref"> (or omit </span><span class="title-ref">command_name</span><span class="title-ref"> for the options shared by all commands like </span><span class="title-ref">--verbosity</span>\`).
...
\$ cd docs \$ optipng -o7 -zm1-9 -i0 -strip all <span class="title-ref">find . -type f -not -path "./\_build/\*" -name "\*.png"</span> \$ advpng -z4 <span class="title-ref">find . -type f -not -path "./\_build/\*" -name "\*.png"</span>
