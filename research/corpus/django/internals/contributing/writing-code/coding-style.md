...
- Unless otherwise specified, follow `8`.

  Use `flake8` to check for problems in this area. Note that our `.flake8` file excludes some errors that we don't consider as gross violations. Remember that `8` is only a guide, so respect the style of the surrounding code as a primary goal.

  An exception to `8` is our rules on line lengths. We allow up to 88 characters in code, as this is the line length used by `black`. Documentation, comments, and docstrings should be wrapped at 79 characters. These limits are checked when `flake8` is run.

- String variable interpolation may use `%-formatting <old-string-formatting>`, `f-strings
  <f-strings>`, or `str.format` as appropriate, with the goal of maximizing code readability.
...
- In docstrings, follow the style of existing docstrings and `257`.

- In tests, use `~django.test.SimpleTestCase.assertRaisesMessage` and `~django.test.SimpleTestCase.assertWarnsMessage` instead of `~unittest.TestCase.assertRaises` and `~unittest.TestCase.assertWarns` so you can check the exception or warning message. Use `~unittest.TestCase.assertRaisesRegex` and `~unittest.TestCase.assertWarnsRegex` only if you need regular expression matching.

  Use `assertIs(…, True/False)<unittest.TestCase.assertIs>` for testing boolean values, rather than `~unittest.TestCase.assertTrue` and `~unittest.TestCase.assertFalse`, so you can check the actual boolean value, not the truthiness of the expression.
...
- Where applicable, use unpacking generalizations compliant with `448`, such as merging mappings (`{**x, **y}`) or sequences (`[*a, *b]`). This improves performance, readability, and maintainability while reducing errors.
...
- If `choices` is defined for a given model field, define each choice as a mapping, with an all-uppercase name as a class attribute on the model. Example:
  ...
  Alternatively, consider using `field-choices-enum-types`:
...
For details about the JavaScript code style used by Django, see `/internals/contributing/writing-code/javascript`.
