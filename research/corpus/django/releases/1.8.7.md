...
- Fixed a data loss possibility with `~django.db.models.Prefetch` if `to_attr` is set to a `ManyToManyField` (`25693`).
...
- Fixed serialization of `~django.contrib.postgres.fields.DateRangeField` and `~django.contrib.postgres.fields.DateTimeRangeField` (`24937`).
