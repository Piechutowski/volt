...
Django relies on user input in some cases (e.g. `django.contrib.auth.views.login()` and `i18n </topics/i18n/index>`) to redirect the user to an "on success" URL. The security check for these redirects (namely `django.utils.http.is_safe_url()`) considered some URLs with basic authentication credentials "safe" when they shouldn't be.
...
- Fixed `~django.contrib.postgres.fields.RangeField` and `~django.contrib.postgres.fields.ArrayField` serialization with `None` values (`26215`).
