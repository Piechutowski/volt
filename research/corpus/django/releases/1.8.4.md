...
The `~django.contrib.sessions.middleware.SessionMiddleware` has been modified to no longer create empty session records, including when `SESSION_SAVE_EVERY_REQUEST` is active.

## Bugfixes

- Added the ability to serialize values from the newly added `~django.db.models.UUIDField` (`25019`).
...
- Moved the `unsaved model instance assignment data loss check
  <unsaved-model-instance-check-18>` to `Model.save()` to allow easier usage of in-memory models (`25160`).
