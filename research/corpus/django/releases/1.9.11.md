...
Older versions of Django don't validate the `Host` header against `settings.ALLOWED_HOSTS` when `settings.DEBUG=True`. This makes them vulnerable to a [DNS rebinding attack](https://benmmurphy.github.io/blog/2016/07/11/rails-webconsole-dns-rebinding/).
