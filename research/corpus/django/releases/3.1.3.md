...
- Fixed displaying Unicode characters in `forms.JSONField <django.forms.JSONField>` and read-only `models.JSONField <django.db.models.JSONField>` values in the admin (`32080`).
...
- Fixed a regression in Django 3.1 where `ProtectedError.protected_objects <django.db.models.ProtectedError>` and `RestrictedError.restricted_objects <django.db.models.RestrictedError>` attributes returned iterators instead of `set` of objects (`32107`).
