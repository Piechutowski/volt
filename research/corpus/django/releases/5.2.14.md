...
<user-uploaded-content-security>` at the web server level rather than solely relying on `FILE_UPLOAD_MAX_MEMORY_SIZE`.
...
Response headers did not `vary on <using-vary-headers>` cookies if a session was not modified, but `SESSION_SAVE_EVERY_REQUEST` was `True`. A remote attacker could steal a user's session after that user visits a cached public page.
