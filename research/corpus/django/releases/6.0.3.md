...
The `~django.forms.URLField` form field's `to_python()` method used `~urllib.parse.urlsplit` to determine whether to prepend a URL scheme to the submitted value. On Windows, `urlsplit()` performs `NFKC normalization <python:unicodedata.normalize>`, which can be disproportionately slow for large inputs containing certain characters.
...
- Fixed `AttributeError` when subclassing builtin lookups and neglecting to `override<tuple-for-params>` `as_sql()` to accept any sequence (`36934`).
