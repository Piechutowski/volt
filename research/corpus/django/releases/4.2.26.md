...
Python's `NFKC normalization <python:unicodedata.normalize>` is slow on Windows. As a consequence, `~django.http.HttpResponseRedirect`, `~django.http.HttpResponsePermanentRedirect`, and the shortcut `redirect() <django.shortcuts.redirect>` were subject to a potential denial-of-service attack via certain inputs with a very large number of Unicode characters (follow up to `2025-27556`).
