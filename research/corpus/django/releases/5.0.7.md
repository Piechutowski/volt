...
Derived classes of the `~django.core.files.storage.Storage` base class which override `generate_filename()
<django.core.files.storage.Storage.generate_filename()>` without replicating the file path validations existing in the parent class, allowed for potential directory-traversal via certain inputs when calling `save()
...
- Fixed a bug in Django 5.0 that caused a crash of `Model.full_clean()` on unsaved model instances with a `GeneratedField` and certain defined `Meta.constraints <django.db.models.Options.constraints>` (`35560`).
