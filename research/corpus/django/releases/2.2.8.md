...
- Fixed a data loss possibility in the admin changelist view when a custom `formset's prefix <formset-prefix>` contains regular expression special characters, e.g. `'$'` (`31031`).
...
- Fixed a data loss possibility in the `~django.db.models.query.QuerySet.select_for_update`. When using `'self'` in the `of` argument with `multi-table inheritance
  <multi-table-inheritance>`, a parent model was locked instead of the queryset's model (`30953`).
