...
Headers containing underscores are now ignored by `ASGIRequest`, matching the behavior of `Daphne <daphne>`, the reference server for ASGI.
...
- Fixed a bug in Django 6.0 where the `fields.E348` system check did not detect name clashes between model managers and `~django.db.models.ForeignKey.related_name`s for non-self-referential relationships (`36973`).
