...
The `django.contrib.auth.handlers.modwsgi.check_password()` function for `authentication via mod_wsgi</howto/deployment/wsgi/apache-auth>` allowed remote attackers to enumerate users via a timing attack.
...
`Raster lookups <spatial-lookup-raster>` on GIS fields (only implemented on PostGIS) were subject to SQL injection if untrusted data was used as a band index.
