...
single: DRY single: Don't repeat yourself
...
This is also why the `FETCH_PEERS` `fetch mode </topics/db/fetch-modes>` exists. It's an optional performance booster for the common case of selecting related objects for every peer in a `QuerySet`.
...
The goal is not to invent a programming language. The goal is to offer just enough programming-esque functionality, such as branching and looping, that is essential for making presentation-related decisions. The `Django Template
...
The cache API should be extensible at the application level based on the developer's needs (for example, see `cache_key_transformation`).
