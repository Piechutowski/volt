...
In general, everything covered in the documentation -- with the exception of anything in the `internals area </internals/index>` is considered stable.
...
If we become aware of a security problem -- hopefully by someone following our `security reporting policy <reporting-security-issues>` -- we'll do everything necessary to fix it. This might mean breaking backwards compatibility; security trumps the compatibility guarantee.
