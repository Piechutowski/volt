...
4.  Check out the rest of the `documentation </index>`, and [ask questions](https://www.djangoproject.com/community/) if you run into trouble.
...
Django comes with its own lightweight development server (`runserver`). For a production environment, Django follows the WSGI spec, `3333`, which means it can run on a variety of web servers. See `/howto/deployment/index` for more information.

Django runs [SQLite](https://www.sqlite.org/) by default, which is included in Python installations. For a production environment, we recommend [PostgreSQL](https://www.postgresql.org/); but we also officially support [MariaDB](https://mariadb.org/), [MySQL](https://www.mysql.com/), [SQLite](https://www.sqlite.org/), and [Oracle](https://www.oracle.com/). See `Supported Databases
...
You can find the latest supported micro version for each series on the [Python download page](https://www.python.org/downloads/).
