...
If you are using `multiple databases</topics/db/multi-db>`, you can use the same interface on each member of the `connections` dictionary:
...
Partially. See `/topics/composite-primary-key`.
...
We try to avoid adding special cases in the Django code to accommodate all the database-specific options such as table type, etc. If you'd like to use any of these options, create a migration with a `~django.db.migrations.operations.RunSQL` operation that contains `ALTER TABLE` statements that do what you want to do.
