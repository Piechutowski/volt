...
The `model reference </topics/db/models>` documentation explains how to use Django's standard field classes -- `~django.db.models.CharField`, `~django.db.models.DateField`, etc. For many purposes, those classes are all you'll need. Sometimes, though, the Django version won't meet your precise requirements, or you'll want to use a field that is entirely different from those shipped with Django.
...
When planning your `~django.db.models.Field` subclass, first give some thought to which existing `~django.db.models.Field` class your new field is most similar to. Can you subclass an existing Django field and save yourself some work? If not, you should subclass the `~django.db.models.Field` class, from which everything is descended.

Initializing your new field is a matter of separating out any arguments that are specific to your case from the common arguments and passing the latter to the `__init__()` method of `~django.db.models.Field` (or your parent class).

In our example, we'll call our field `HandField`. (It's a good idea to call your `~django.db.models.Field` subclass `<Something>Field`, so it's easily identifiable as a `~django.db.models.Field` subclass.) It doesn't behave like any existing field, so we'll subclass directly from `~django.db.models.Field`:
...
Many of Django's model fields accept options that they don't do anything with. For example, you can pass both `~django.db.models.Field.editable` and `~django.db.models.DateField.auto_now` to a `django.db.models.DateField` and it will ignore the `~django.db.models.Field.editable` parameter (`~django.db.models.DateField.auto_now` being set implies `editable=False`). No error is raised in this case.
...
- `~django.db.models.Field.default`
- `~django.db.models.Field.editable`
...
- `~django.db.models.Field.unique_for_date`
- `~django.db.models.Field.unique_for_month`
- `~django.db.models.Field.unique_for_year`
- `~django.db.models.Field.choices`
- `~django.db.models.Field.help_text`
- `~django.db.models.Field.db_column`
- `~django.db.models.Field.db_tablespace`: Only for index creation, if the backend supports `tablespaces </topics/db/tablespaces>`. You can usually ignore this option.
- `~django.db.models.Field.auto_created`: `True` if the field was automatically created, as for the `~django.db.models.OneToOneField` used by model inheritance. For advanced use only.

All of the options without an explanation in the above list have the same meaning they do for normal Django fields. See the `field documentation
</ref/models/fields>` for examples and details.

### Field deconstruction

The counterpoint to writing your `__init__()` method is writing the `~.Field.deconstruct` method. It's used during `model migrations
</topics/migrations>` to tell Django how to take an instance of your new field and reduce it to a serialized form - in particular, what arguments to pass to `__init__()` to recreate it.
...
As discussed in `removing fields <migrations-removing-model-fields>`, you must retain the original `CustomCharField` class as long as you have migrations that reference it.

### Documenting your custom field

As always, you should document your field type, so users will know what it is. In addition to providing a docstring for it, which is useful for developers, you can also allow users of the admin app to see a short description of the field type via the `django.contrib.admindocs
</ref/contrib/admin/admindocs>` application. To do this provide descriptive text in a `~Field.description` class attribute of your custom field. In the above example, the description displayed by the `admindocs` application for a `HandField` will be 'A hand of cards (bridge style)'.

In the `django.contrib.admindocs` display, the field description is interpolated with `field.__dict__` which allows the description to incorporate arguments of the field. For example, the description for `~django.db.models.CharField` is:

    description = _("String (up to %(max_length)s)")

### Useful methods

Once you've created your `~django.db.models.Field` subclass, you might consider overriding a few standard methods, depending on your field's behavior. The list of methods below is in approximately decreasing order of importance, so start from the top.

#### Custom database types

Say you've created a PostgreSQL custom type called `mytype`. You can subclass `Field` and implement the `~Field.db_type` method, like so:
...
The `~Field.db_type` and `~Field.rel_db_type` methods are called by Django when the framework constructs the `CREATE TABLE` statements for your application -- that is, when you first create your tables. The methods are also called when constructing a `WHERE` clause that includes the model field --that is, when you retrieve data using QuerySet methods like `get()`, `filter()`, and `exclude()` and have the model field as an argument.
...
The `~Field.rel_db_type` method is called by fields such as `ForeignKey` and `OneToOneField` that point to another field to determine their database column data types. For example, if you have an `UnsignedAutoField`, you also need the foreign keys that point to that field to use the same data type:
...
If your custom `~Field` class deals with data structures that are more complex than strings, dates, integers, or floats, then you may need to override `~Field.from_db_value` and `~Field.to_python`.

If present for the field subclass, `from_db_value()` will be called in all circumstances when the data is loaded from the database, including in aggregates and `~django.db.models.query.QuerySet.values` calls.

`to_python()` is called by deserialization and during the `~django.db.models.Model.clean` method used from forms.
...
For `to_python()`, if anything goes wrong during value conversion, you should raise a `~django.core.exceptions.ValidationError` exception.

#### Converting Python objects to query values

Since using a database requires conversion in both ways, if you override `~Field.from_db_value` you also have to override `~Field.get_prep_value` to convert Python objects back to query values.
...
Some data types (for example, dates) need to be in a specific format before they can be used by a database backend. `~Field.get_db_prep_value` is the method where those conversions should be made. The specific connection that will be used for the query is passed as the `connection` parameter. This allows you to use backend-specific conversion logic if it is required.
...
In case your custom field needs a special conversion when being saved that is not the same as the conversion used for normal query parameters, you can override `~Field.get_db_prep_save`.

#### Preprocessing values before saving

If you want to preprocess the value just before saving, you can use `~Field.pre_save`. For example, Django's `~django.db.models.DateTimeField` uses this method to set the attribute correctly in the case of `~django.db.models.DateField.auto_now` or `~django.db.models.DateField.auto_now_add`.
...
To customize the form field used by `~django.forms.ModelForm`, you can override `~Field.formfield`.

The form field class can be specified via the `form_class` and `choices_form_class` arguments; the latter is used if the field has choices specified, the former otherwise. If these arguments are not provided, `~django.forms.CharField` or `~django.forms.TypedChoiceField` will be used.

All of the `kwargs` dictionary is passed directly to the form field's `__init__()` method. Normally, all you need to do is set up a good default for the `form_class` (and maybe `choices_form_class`) argument and then delegate further handling to the parent class. This might require you to write a custom form field (and even a form widget). See the `forms documentation
</topics/forms/index>` for information about this.

If you wish to exclude the field from the `~django.forms.ModelForm`, you can override the `~Field.formfield` method to return `None`.

Continuing our ongoing example, we can write the `~Field.formfield` method as:
...
If `.get_internal_type` returns a string that is not known to Django for the database backend you are using -- that is, it doesn't appear in `django.db.backends.<db_name>.base.DatabaseWrapper.data_types` -- the string will still be used by the serializer, but the default `~Field.db_type` method will return `None`. See the documentation of `~Field.db_type` for reasons why this might be useful. Putting a descriptive string in as the type of the field for the serializer is a useful idea if you're ever going to be using the serializer output in some other place, outside of Django.

#### Converting field data for serialization

To customize how the values are serialized by a serializer, you can override `~Field.value_to_string`. Using `~Field.value_from_object` is the best way to get the field's value prior to serialization. For example, since `HandField` uses strings for its data storage anyway, we can reuse some existing conversion code:
