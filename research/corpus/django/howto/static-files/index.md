...
3.  In your templates, use the `static` template tag to build the URL for the given relative path using the configured `staticfiles` `STORAGES` alias.
    ...
    <div id="staticfiles-in-templates">
...
See `/howto/static-files/deployment` for proper strategies to serve static files in production environments.
...
You can namespace static assets in `STATICFILES_DIRS` by specifying `prefixes <staticfiles-dirs-prefixes>`.
...
This is not suitable for production use! For some common deployment strategies, see `/howto/static-files/deployment`.
...
<django.test.LiveServerTestCase>`) the static assets need to be served along the rest of the content so the test environment reproduces the real one as faithfully as possible, but `LiveServerTestCase` has only very basic static file-serving functionality: It doesn't know about the finders feature of the `staticfiles` application and assumes the static content has already been collected under `STATIC_ROOT`.
...
3.  Use a web server of your choice to serve the files. `/howto/static-files/deployment` covers some common deployment strategies for static files.

## Learn more

This document has covered the basics and some common usage patterns. For complete details on all the settings, commands, template tags, and other pieces included in `django.contrib.staticfiles`, see `the staticfiles
