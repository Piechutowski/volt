...
[Granian](https://github.com/emmett-framework/granian) is a Rust-based high-performance HTTP server for Python applications, supporting ASGI and WSGI interfaces.
...
When Granian is installed, a `granian` command is available which runs WSGI applications. Granian needs to be called with the location of a module containing a WSGI application object named `application`, using the `--interface wsgi` option.
