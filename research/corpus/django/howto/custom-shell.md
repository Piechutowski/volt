...
The customization above adds `~django.urls.resolve` and `~django.urls.reverse` to the default namespace, which already includes all models from the apps listed in `INSTALLED_APPS` plus what is imported by default. These objects will be available in the `shell` without requiring a manual import.
...
Note that automatic imports can be disabled for a specific `shell` session using the `--no-imports <shell --no-imports>` flag. To permanently disable automatic imports, override `get_auto_imports()` to return `None`:
