...
- `logging-how-to`
- `Django logging overview <logging-explanation>`

</div>

<div class="module" synopsis="Logging tools for Django applications">
...
- The `django-server-logger` logger sends messages at the `INFO` level or higher to the console.

All loggers except `django-server-logger` propagate logging to their parents, up to the root `django` logger. The `console` and `mail_admins` handlers are attached to the root logger to provide the behavior described above.
...
<naming-loggers-hierarchy>`. Django does not post messages using this name. Instead, it uses one of the loggers below.
...
Log messages related to `/ref/contrib/auth`, particularly `ERROR` messages are generated when a `~django.contrib.auth.forms.PasswordResetForm` is successfully submitted but the password reset email cannot be delivered due to a mail sending exception.

#### `django.contrib.gis`

Log messages related to `/ref/contrib/gis/index` at various points: during the loading of external GeoSpatial libraries (GEOS, GDAL, etc.) and when reporting errors. Each `ERROR` log record includes the caught exception and relevant contextual data.

#### `django.dispatch`

This logger is used in `/ref/signals`, specifically within the `~django.dispatch.Signal` class, to report issues when dispatching a signal to a connected receiver. The `ERROR` log record includes the caught exception as `exc_info` and adds the following extra context:
...
- `django.security.csrf`: For `CSRF failures <csrf-rejected-requests>`.

#### `django.db.backends.schema`

Logs the SQL queries that are executed during schema changes to the database by the `migrations framework </topics/migrations>`. Note that it won't log the queries executed by `~django.db.migrations.operations.RunPython`. Messages to this logger have `params` and `sql` in their extra context (but unlike `django.db.backends`, not duration). The values have the same meaning as explained in `django-db-logger`.
...
<div class="AdminEmailHandler(include_html=False, email_backend=None, reporter_class=None, using=None)">
...
Be aware of the `security implications of logging
<logging-security-implications>` when using the `AdminEmailHandler`.

Email is sent using the default `mailer <topic-email-configuration>`. This can be overridden by setting the `using` argument of `AdminEmailHandler`, like this:
...
By setting the deprecated `email_backend` argument of `AdminEmailHandler`, the `email backend <topic-email-backends>` that is being used by the handler can be overridden. `email_backend` is not supported when `MAILERS` is defined or when the `using` argument is provided.
...
Sends emails to admin users. To customize this behavior, you can subclass the `~django.utils.log.AdminEmailHandler` class and override this method.
...
<div class="CallbackFilter(callback)">
...
For instance, to filter out `~django.http.UnreadablePostError` (raised when a user cancels an upload) from the admin emails, you would create a filter function:
