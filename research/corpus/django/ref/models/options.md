...
This document explains all the possible `metadata options
<meta-options>` that you can give your model in its internal `class Meta`.
...
If `abstract = True`, this model will be an `abstract base class <abstract-base-classes>`.
...
The attribute name of the manager, for example, `'objects'`, to use for the model's `~django.db.models.Model._base_manager`.
...
To save you time, Django automatically derives the name of the database table from the name of your model class and the app that contains it. A model's database table name is constructed by joining the model's "app label" -- the name you used in `manage.py startapp <startapp>` -- to the model's class name, with an underscore between them.
...
It is strongly advised that you use lowercase table names when you override the table name via `db_table`, particularly if you are using the MySQL backend. See the `MySQL notes <mysql-notes>` for more details.
...
Such quoted names can also be used with Django's other supported database backends; except for Oracle, however, the quotes have no effect. See the `Oracle notes <oracle-notes>` for more details.
...
The name of the manager to use for the model's `~django.db.models.Model._default_manager`.
...
The name of a field or a list of field names in the model, typically `DateField`, `DateTimeField`, or `IntegerField`. This specifies the default field(s) to use in your model `Manager`’s `~django.db.models.query.QuerySet.latest` and `~django.db.models.query.QuerySet.earliest` methods.
...
See the `~django.db.models.query.QuerySet.latest` docs for more.
...
2.  If a model with `managed=False` contains a `~django.db.models.ManyToManyField` that points to another unmanaged model, then the intermediate table for the many-to-many join will also not be created. However, the intermediary table between one managed and one unmanaged model *will* be created.
...
You can also use `query expressions </ref/models/expressions>`. To order by `author` ascending and make null values sort last, use this:
...
In `GROUP BY queries <aggregation-ordering-interaction>` (for example, those using `~.QuerySet.values` and `~.QuerySet.annotate`), the default ordering is not applied.
...
If `proxy = True`, a model which subclasses another model will be treated as a `proxy model <proxy-models>`.
...
A list of `indexes </ref/models/indexes>` that you want to define on the model:
...
Use `.UniqueConstraint` with the `~Options.constraints` option instead.
...
A `~django.db.models.ManyToManyField` cannot be included in `unique_together`. (It's not clear what that would even mean!) If you need to validate uniqueness related to a `~django.db.models.ManyToManyField`, try using a signal or an explicit `through <ManyToManyField.through>` model.
...
A list of `constraints </ref/models/constraints>` that you want to define on the model:
...
If this isn't given, Django will use `~Options.verbose_name` + `"s"`.
