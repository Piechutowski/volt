...
<div class="module" synopsis="Model meta-class layer">
...
`field_name` can be the name of a field on the model, a field on an abstract or inherited model, or a field defined on another model that points to the model. In the latter case, the `field_name` will be (in order of preference) the `~.ForeignKey.related_query_name` set by the user, the `~.ForeignKey.related_name` set by the user, or the name automatically generated by Django.

Hidden fields, fields with `hidden=True
<django.db.models.Field.hidden>`, cannot be retrieved by name.

If a field with the given name is not found a `~django.core.exceptions.FieldDoesNotExist` exception will be raised.
...
`include_hidden`  
`False` by default. If set to `True`, `get_fields()` will include fields with `hidden=True <django.db.models.Field.hidden>`.
...
When a `~django.db.models.CompositePrimaryKey` is defined on a model it will contain all the fields referenced by it.
...
Otherwise it will contain the single field declared as the primary key of the model, either explicitly with `primary_key=True
<django.db.models.Field.primary_key>` or implicitly as the `automatic
