...
<div class="module" synopsis="Django's built-in utilities.">
...
<div class="module" synopsis="Helper functions for controlling caching.">
...
For information on the `Vary` header, see `9110#section-12.5.5`.
...
For example, `internationalization </topics/i18n/index>` middleware would need to distinguish caches by the `Accept-language` header.
...
Adds (or updates) the `Vary` header in the given `HttpResponse` object. `newheaders` is a list of header names that should be in `Vary`. If headers contains an asterisk, then `Vary` header will consist of a single asterisk `'*'`, according to `9110#section-12.5.5`. Otherwise, existing headers in `Vary` aren't removed.
...
<div class="module" synopsis="Functions to parse strings to datetime objects.">
...
Years and months not supported

ISO 8601 years (`Y`) and months (`M`) are unsupported as they cannot be represented by `~datetime.timedelta`.
...
<div class="module" synopsis="Functions that help with creating decorators for views.">
...
See `decorating class based views <decorating-class-based-views>` for example usage.
...
Like `decorator_from_middleware`, but returns a function that accepts the arguments to be passed to the middleware_class. For example, the `~django.views.decorators.cache.cache_page` decorator is created from the `CacheMiddleware` like this:
...
Marks a middleware as `synchronous-only <async-middleware>`. (The default in Django, but this allows you to future-proof if the default ever changes in a future release.)
...
Marks a middleware as `asynchronous-only <async-middleware>`. Django will wrap it in an asynchronous event loop when it is called from the WSGI request path.
...
Marks a middleware as `sync and async compatible <async-middleware>`, this allows to avoid converting requests. You must implement detection of the current request type to use this decorator. See `asynchronous
...
<div class="module" synopsis="A series of helper functions to manage character encoding.">
...
This is the algorithm from section 3.1 of `3987#section-3.1`, slightly simplified since the input is assumed to be a string rather than an arbitrary byte stream.
...
This is an algorithm from section 3.2 of `3987#section-3.2`.
...
Convert a file system path to a URI portion that is suitable for inclusion in a URL. The path is assumed to be either UTF-8 bytes, string, or a `~pathlib.Path`.
...
<div class="module" synopsis="Syndication feed generation library -- used for generating RSS,
etc.">
...
For definitions of the different versions of RSS, see [The myth of RSS compatibility](https://web.archive.org/web/20110718035220/http://diveintomark.org/archives/2004/02/04/incompatible-rss).
...
<div class="Stylesheet(url, mimetype="", media="screen")">
...
An optional string which will be used as the `media` attribute of the stylesheet. Defaults to `"screen"`. Use `media=None` if you don't want your stylesheet to have a `media` attribute.
...
<div class="SyndicationFeed">

Base class for all syndication feeds. Subclasses should provide `write()`.

<div class="method">

\_\_init\_\_(title, link, description, language=None, author_email=None, author_name=None, author_link=None, subtitle=None, categories=None, feed_url=None, feed_copyright=None, feed_guid=None, ttl=None, stylesheets=None, \*\*kwargs)
...
<div class="Rss201rev2Feed(RssFeed)">
...
<div class="RssUserland091Feed(RssFeed)">
...
<div class="Atom1Feed(SyndicationFeed)">
...
<div class="module" synopsis="Functional programming tools.">

django.utils.functional

</div>

<div class="cached_property(func)">
...
<descriptor-invocation>` works, using `del` (or `delattr`) on a `cached_property` that hasn't been accessed raises `AttributeError`.
...
<div class="classproperty(method=None)">

Similar to `@classmethod <classmethod>`, the `@classproperty` decorator converts the result of a method with a single `cls` argument into a property that can be accessed directly from the class.
...
<div class="module" synopsis="HTML helper functions">
...
Returns the given text with ampersands, quotes and angle brackets encoded for use in HTML. The input is first coerced to a string and the output has `~django.utils.safestring.mark_safe` applied.
...
This is similar to `str.format`, except that it is appropriate for building up HTML fragments. The first argument `format_string` is not escaped but all other args and kwargs are passed through `conditional_escape` before being passed to `str.format()`. Finally, the output has `~django.utils.safestring.mark_safe` applied.
...
Note that although this function uses `str.format()` to do the interpolation, some of the formatting options provided by `str.format()` (e.g. number formatting) will not work, since all arguments are passed through `conditional_escape` which (ultimately) calls `~django.utils.encoding.force_str` on the values.
...
Absolutely NO guarantee is provided about the resulting string being HTML safe. So NEVER mark safe the result of a `strip_tags` call without escaping it first, for example with `~django.utils.html.escape`.
...
This decorator defines the `__html__()` method on the decorated class by wrapping `__str__()` in `~django.utils.safestring.mark_safe`. Ensure the `__str__()` method does indeed return text that doesn't require HTML escaping.

</div>

## `django.utils.http`

<div class="module" synopsis="HTTP helper functions. (URL encoding, cookie handling, ...)">
...
Formats the time to match the `1123#section-5.2.14` date format as specified by HTTP `9110#section-5.6.7`.
...
Constructs a `Content-Disposition` HTTP header value from the given `filename` as specified by `6266`. Returns `None` if `as_attachment` is `False` and `filename` is `None`, otherwise returns a string suitable for the `Content-Disposition` HTTP header.
...
<div class="module" synopsis="Functions for working with Python modules.">
...
<div class="module" synopsis="Functions and classes for working with strings that can be
...
<div class="module" synopsis="Text manipulation.">
...
<div class="module" synopsis="Timezone support.">
...
Returns a `~datetime.tzinfo` instance that represents a time zone with a fixed offset from UTC.
...
Returns a `~datetime.tzinfo` instance that represents the `default time zone <default-current-time-zone>`.
...
Returns a `~datetime.tzinfo` instance that represents the `current time zone <default-current-time-zone>`.
...
Sets the `current time zone <default-current-time-zone>`. The `timezone` argument must be an instance of a `~datetime.tzinfo` subclass or a time zone name.
...
Unsets the `current time zone <default-current-time-zone>`.
...
This is a Python context manager that sets the `current time zone
<default-current-time-zone>` on entry with `activate`, and restores the previously active time zone on exit. If the `timezone` argument is `None`, the `current time zone <default-current-time-zone>` is unset on entry with `deactivate` instead.
...
Converts an aware `~datetime.datetime` to a different time zone, by default the `current time zone <default-current-time-zone>`.
...
Uses `localtime` to convert an aware `~datetime.datetime` to a `~datetime.datetime.date` in a different time zone, by default the `current time zone <default-current-time-zone>`.
...
Returns a `~datetime.datetime` that represents the current point in time. Exactly what's returned depends on the value of `USE_TZ`:

- If `USE_TZ` is `False`, this will be a `naive <naive_vs_aware_datetimes>` datetime (i.e. a datetime without an associated timezone) that represents the current time in the system's local timezone.
- If `USE_TZ` is `True`, this will be an `aware <naive_vs_aware_datetimes>` datetime representing the current time in UTC. Note that `now` will always return times in UTC regardless of the value of `TIME_ZONE`; you can use `localtime` to get the time in the current time zone.
...
Returns `True` if `value` is aware, `False` if it is naive. This function assumes that `value` is a `~datetime.datetime`.
...
Returns `True` if `value` is naive, `False` if it is aware. This function assumes that `value` is a `~datetime.datetime`.
...
Returns an aware `~datetime.datetime` that represents the same point in time as `value` in `timezone`, `value` being a naive `~datetime.datetime`. If `timezone` is set to `None`, it defaults to the `current time zone <default-current-time-zone>`.
...
Returns a naive `~datetime.datetime` that represents in `timezone` the same point in time as `value`, `value` being an aware `~datetime.datetime`. If `timezone` is set to `None`, it defaults to the `current time zone <default-current-time-zone>`.

</div>

## `django.utils.translation`

<div class="module" synopsis="Internationalization support.">

django.utils.translation

</div>

For a complete discussion on the usage of the following see the `translation documentation </topics/i18n/translation>`.
...
See `lazy translations documentation <lazy-translations>`.
