...
- `date_list`: A `QuerySet <django.db.models.query.QuerySet.dates>` object containing all years that have objects available according to `queryset`, represented as `datetime.datetime` objects, in descending order.
...
Determine if an object list will be returned as part of the context. Returns `~YearArchiveView.make_object_list` by default.
...
- `date_list`: A `QuerySet <django.db.models.query.QuerySet.dates>` object containing all months that have objects available according to `queryset`, represented as `datetime.datetime` objects, in ascending order.
- `year`: A `~datetime.date` object representing the given year.
- `next_year`: A `~datetime.date` object representing the first day of the next year, according to `~BaseDateListView.allow_empty` and `~DateMixin.allow_future`.
- `previous_year`: A `~datetime.date` object representing the first day of the previous year, according to `~BaseDateListView.allow_empty` and `~DateMixin.allow_future`.
...
- `date_list`: A `QuerySet <django.db.models.query.QuerySet.dates>` object containing all days that have objects available in the given month, according to `queryset`, represented as `datetime.datetime` objects, in ascending order.
- `month`: A `~datetime.date` object representing the given month.
- `next_month`: A `~datetime.date` object representing the first day of the next month, according to `~BaseDateListView.allow_empty` and `~DateMixin.allow_future`.
- `previous_month`: A `~datetime.date` object representing the first day of the previous month, according to `~BaseDateListView.allow_empty` and `~DateMixin.allow_future`.
...
- `next_week`: A `~datetime.date` object representing the first day of the next week, according to `~BaseDateListView.allow_empty` and `~DateMixin.allow_future`.
- `previous_week`: A `~datetime.date` object representing the first day of the previous week, according to `~BaseDateListView.allow_empty` and `~DateMixin.allow_future`.

**Notes**

- Uses a default `template_name_suffix` of `_archive_week`.
- The `week_format` attribute is a `~time.strptime` format string used to parse the week number. The following values are supported:
...
- `next_day`: A `~datetime.date` object representing the next day, according to `~BaseDateListView.allow_empty` and `~DateMixin.allow_future`.
- `previous_day`: A `~datetime.date` object representing the previous day, according to `~BaseDateListView.allow_empty` and `~DateMixin.allow_future`.
- `next_month`: A `~datetime.date` object representing the first day of the next month, according to `~BaseDateListView.allow_empty` and `~DateMixin.allow_future`.
- `previous_month`: A `~datetime.date` object representing the first day of the previous month, according to `~BaseDateListView.allow_empty` and `~DateMixin.allow_future`.
...
All of the generic views listed above have matching `Base` views that only differ in that they do not include the `~django.views.generic.list.MultipleObjectTemplateResponseMixin` (for the archive views) or `~django.views.generic.detail.SingleObjectTemplateResponseMixin` (for the `DateDetailView`):

<div class="BaseArchiveIndexView">

<div class="BaseYearArchiveView">

<div class="BaseMonthArchiveView">

<div class="BaseWeekArchiveView">

<div class="BaseDayArchiveView">

<div class="BaseTodayArchiveView">
