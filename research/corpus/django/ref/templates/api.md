...
This document explains the Django template system from a technical perspective -- how it works and how to extend it. If you're looking for reference on the language syntax, see `/ref/templates/language`.

It assumes an understanding of templates, contexts, variables, tags, and rendering. Start with the `introduction to the Django template language
...
Django projects generally rely on the `high level, backend agnostic APIs
<template-engines>` for each of these steps instead of the template system's lower level APIs:

1.  For each `~django.template.backends.django.DjangoTemplates` backend in the `TEMPLATES` setting, Django instantiates an `Engine`. `~django.template.backends.django.DjangoTemplates` wraps `Engine` and adapts it to the common template backend API.
2.  The `django.template.loader` module provides functions such as `~django.template.loader.get_template` for loading templates. They return a `django.template.backends.django.Template` which wraps the actual `django.template.Template`.
3.  The `Template` obtained in the previous step has a `~django.template.backends.base.Template.render` method which marshals a context and possibly a request into a `Context` and delegates the rendering to the underlying `Template`.
...
<div class="Engine(dirs=None, app_dirs=False, context_processors=None, debug=False, loaders=None, string_if_invalid='', file_charset='utf-8', libraries=None, builtins=None, autoescape=True)">

When instantiating an `Engine` all arguments must be passed as keyword arguments:

- `dirs` is a list of directories where the engine should look for template source files. It is used to configure `filesystem.Loader <django.template.loaders.filesystem.Loader>`.
...
Returns the underlying `Engine` from the first configured `~django.template.backends.django.DjangoTemplates` engine. Raises `~django.core.exceptions.ImproperlyConfigured` if no engines are configured.
...
Like `~Engine.get_template`, except it takes a list of names and returns the first template that was found.

</div>

## Loading a template

The recommended way to create a `Template` is by calling the factory methods of the `Engine`: `~Engine.get_template`, `~Engine.select_template` and `~Engine.from_string`.

In a Django project where the `TEMPLATES` setting defines a `~django.template.backends.django.DjangoTemplates` engine, it's possible to instantiate a `Template` directly. If more than one `~django.template.backends.django.DjangoTemplates` engine is defined, the first one will be used.
...
<div class="Context(dict_=None, autoescape=True, use_l10n=None, use_tz=None)">
...
- `use_l10n` overrides whether values will be localized by default. If set to `True` numbers and dates will be formatted based on locale.
  ...
  See `topic-l10n-templates` for details.
...
- `use_tz` overrides whether dates are converted to the local time when rendered in a template. If set to `True` all dates will be rendered using the local timezone. This takes precedence over `USE_TZ`.
  ...
  See `time-zones-in-templates` for details.

For example usage, see `playing-with-context` below.
...
<div id="alters-data-description">
...
- There can be side effects when calling some variables, and it'd be either foolish or a security hole to allow the template system to access them.
  ...
  A good example is the `~django.db.models.Model.delete` method on each Django model object. The template system shouldn't be allowed to do something like this:

  ``` html+django
  I will now delete this valuable data. {{ data.delete }}
  ```

  To prevent this, set an `alters_data` attribute on the callable variable. The template system won't call a variable if it has `alters_data=True` set, and will instead replace the variable with `string_if_invalid`, unconditionally. The dynamically-generated `~django.db.models.Model.delete` and `~django.db.models.Model.save` methods on Django model objects get `alters_data=True` automatically. Example:
...
<div class="RequestContext(request, dict_=None, processors=None, use_l10n=None, use_tz=None, autoescape=True)">

Django comes with a special `~django.template.Context` class, `django.template.RequestContext`, that acts slightly differently from the normal `django.template.Context`. The first difference is that it takes an `~django.http.HttpRequest` as its first argument. For example:
...
Django does this to allow context data to override context processors in APIs such as `~django.shortcuts.render` and `~django.template.response.TemplateResponse`.
...
See `i18n template tags <i18n-template-tags>` for template tags that generate the same values.
...
If this processor is enabled, every `RequestContext` will contain a variable `csp_nonce`, providing a securely generated, request-specific nonce suitable for use under a Content Security Policy. See `CSP nonce usage <csp-nonce>` for details.

#### `django.template.context_processors.request`

If this processor is enabled, every `RequestContext` will contain a variable `request`, which is the current `~django.http.HttpRequest`.
...
- `messages` -- A list of messages (as strings) that have been set via the `messages framework </ref/contrib/messages>`.
- `DEFAULT_MESSAGE_LEVELS` -- A mapping of the message level names to `their numeric value <message-level-constants>`.

### Writing your own context processors

A context processor has a simple interface: It's a Python function that takes one argument, an `~django.http.HttpRequest` object, and returns a dictionary that gets added to the template context.
...
Custom context processors can live anywhere in your code base. All Django cares about is that your custom context processors are pointed to by the `'context_processors'` option in your `TEMPLATES` setting — or the `context_processors` argument of `~django.template.Engine` if you're using it directly.

## Loading templates

Generally, you'll store templates in files on your filesystem rather than using the low-level `~django.template.Template` API yourself. Save templates in a directory specified as a **template directory**.
...
### The `DIRS <TEMPLATES-DIRS>` option

Tell Django what your template directories are by using the `DIRS
<TEMPLATES-DIRS>` option in the `TEMPLATES` setting in your settings file — or the `dirs` argument of `~django.template.Engine`. This should be set to a list of strings that contain full paths to your template directories:
...
<div class="app_directories.Loader">
...
You can enable this loader by setting `APP_DIRS
...
<div id="custom-template-loaders">
...
A method that takes a `template_name` and yields `~django.template.base.Origin` instances for each possible source.
...
Returns the contents for a template given a `~django.template.base.Origin` instance.

This is where a filesystem loader would read contents from the filesystem, or a database loader would read from the database. If a matching template doesn't exist, this should raise a `~django.template.TemplateDoesNotExist` error.
...
Returns a `Template` object for a given `template_name` by looping through results from `get_template_sources` and calling `get_contents`. This returns the first matching template. If no template is found, `~django.template.TemplateDoesNotExist` is raised.
...
<div class="Origin(name, template_name=None, loader=None)">
