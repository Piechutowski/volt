...
<div class="module" synopsis="Provides cookie- and session-based temporary message storage.">
...
Messages are implemented through a `middleware </ref/middleware>` class and corresponding `context processor </ref/templates/api>`.
...
- `MIDDLEWARE` contains `'django.contrib.sessions.middleware.SessionMiddleware'` and `'django.contrib.messages.middleware.MessageMiddleware'`.
  ...
  The default `storage backend <message-storage-backends>` relies on `sessions </topics/http/sessions>`. That's why `SessionMiddleware` must be enabled and appear before `MessageMiddleware` in `MIDDLEWARE`.
...
<div class="storage.session.SessionStorage">
...
<div class="storage.cookie.CookieStorage">
...
<div class="storage.fallback.FallbackStorage">
...
`~django.contrib.messages.storage.fallback.FallbackStorage` is the default storage class. If it isn't suitable to your needs, you can select another storage class by setting `MESSAGE_STORAGE` to its full import path, for example:

    MESSAGE_STORAGE = "django.contrib.messages.storage.cookie.CookieStorage"

<div class="storage.base.BaseStorage">
...
**Outside of templates**, you can use `~django.contrib.messages.get_messages`:
...
For instance, you can fetch all the messages to return them in a `JSONResponseMixin <jsonresponsemixin-example>` instead of a `~django.views.generic.base.TemplateResponseMixin`.

`~django.contrib.messages.get_messages` will return an instance of the configured storage backend.
...
- `level`: An integer describing the type of the message (see the [message levels](#message-levels) section above).
...
<div id="message-level-constants">
...
For more information on how the minimum recorded level functions, see [Message levels](#message-levels) above.
...
<div class="views.SuccessMessageMixin">

Adds a success message attribute to `~django.views.generic.edit.FormView` based classes
...
The cleaned data from the `form` is available for string interpolation using the `%(field_name)s` syntax. For ModelForms, if you need access to fields from the saved `object` override the `~django.contrib.messages.views.SuccessMessageMixin.get_success_message` method.
...
A few `settings<settings-messages>` give you control over message behavior:
...
Asserts that `~django.contrib.messages` added to the `response
<django.http.HttpResponse>` matches `expected_messages`.

`expected_messages` is a list of `~django.contrib.messages.Message` objects.
