...
<div class="module" synopsis="A framework for generating syndication feeds, in RSS and Atom,
quite easily.">
...
Django comes with a high-level syndication-feed-generating framework for creating [RSS](https://developer.mozilla.org/en-US/docs/Glossary/RSS) and `Atom <4287>` feeds.
...
The high-level feed-generating framework is supplied by the `~django.contrib.syndication.views.Feed` class. To create a feed, write a `~django.contrib.syndication.views.Feed` class and point to an instance of it in your `URLconf
</topics/http/urls>`.
...
Instances of `~django.contrib.syndication.views.Feed` classes are views which can be used in your `URLconf </topics/http/urls>`.
...
To connect a URL to this feed, put an instance of the Feed object in your `URLconf </topics/http/urls>`. For example:
...
- `items()` is, a method that returns a list of objects that should be included in the feed as `<item>` elements. Although this example returns `NewsItem` objects using Django's `object-relational mapper
  </ref/models/querysets>`, `items()` doesn't have to return model instances. Although you get a few bits of functionality "for free" by using Django models, `items()` can return any type of object you want.
...
These can be matched with a `URLconf </topics/http/urls>` line such as:
...
The `ExampleFeed` class below gives full documentation on methods and attributes of `~django.contrib.syndication.views.Feed` classes.

### Specifying the type of feed

By default, feeds produced in this framework use RSS 2.0.

To change that, add a `feed_type` attribute to your `~django.contrib.syndication.views.Feed` class, like so:
...
The `link` method/attribute can return either an absolute path (e.g. `"/blog/"`) or a URL with the fully-qualified domain and protocol (e.g. `"https://www.example.com/blog/"`). If `link` doesn't return the domain, the syndication framework will insert the domain of the current site, according to your `SITE_ID setting <SITE_ID>`.
...
Some developers like to make available both Atom *and* RSS versions of their feeds. To do that, you can create a subclass of your `~django.contrib.syndication.views.Feed` class and set the `feed_type` to something different. Then update your URLconf to add the extra versions.
...
If you provide a `description` in your `~django.contrib.syndication.views.Feed` class, Django will *not* automatically put that into the `subtitle` element, because a subtitle and description are not necessarily the same thing. Instead, you should define a `subtitle` attribute.
...
This example illustrates all possible attributes and methods for a `~django.contrib.syndication.views.Feed` class:
...
The `~django.utils.feedgenerator` module contains a base class:
...
- `stylesheets` should be a sequence of either strings or `~django.utils.feedgenerator.Stylesheet` instances.
...
- `pubdate` should be a Python `~datetime.datetime` object.
- `updateddate` should be a Python `~datetime.datetime` object.
...
Callback to add elements inside the root feed element (`feed`/`channel`). `handler` is an `~xml.sax.saxutils.XMLGenerator` from Python's built-in SAX library; you'll call methods on it to add to the XML document in process.
...
If you wish to have your RSS feed render nicely in a browser, you will need to provide styling information for the XML file, typically in [XSLT](https://developer.mozilla.org/en-US/docs/Web/XSLT/Transforming_XML_with_XSLT) or CSS formats.
...
Django will normally try to guess the MIME type of the given URL based on its extension, but if that fails you can specify it using the `~django.utils.feedgenerator.Stylesheet` class:
...
Similarly, if you'd like to use a different `media` attribute than `screen` (Django's default), you can use the `~django.utils.feedgenerator.Stylesheet` class again:
