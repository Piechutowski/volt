...
Included in this documentation are some additional notes and settings for `testing-postgis` users.
...
To have the GeoDjango tests executed when `running the Django test suite
<running-unit-tests>` with `runtests.py` all of the databases in the settings file must be using one of the `spatial database backends
<spatial-backends>`.
