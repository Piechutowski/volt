...
To use it, add `'django.contrib.postgres'` in your `INSTALLED_APPS` and activate the [pg_trgm extension](https://www.postgresql.org/docs/current/pgtrgm.html) on PostgreSQL. You can install the extension using the `~django.contrib.postgres.operations.TrigramExtension` migration operation.

The `trigram_similar` lookup can be used on `~django.db.models.CharField` and `~django.db.models.TextField`:
...
The `trigram_word_similar` lookup can be used on `~django.db.models.CharField` and `~django.db.models.TextField`:
...
The `trigram_strict_word_similar` lookup can be used on `~django.db.models.CharField` and `~django.db.models.TextField`.
...
This lookup is implemented using `~django.db.models.Transform`, so it can be chained with other lookup functions. To use it, you need to add `'django.contrib.postgres'` in your `INSTALLED_APPS` and activate the [unaccent extension on PostgreSQL](https://www.postgresql.org/docs/current/unaccent.html). The `~django.contrib.postgres.operations.UnaccentExtension` migration operation is available if you want to perform this activation using migrations.

The `unaccent` lookup can be used on `~django.db.models.CharField` and `~django.db.models.TextField`:
