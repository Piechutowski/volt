...
<div class="module" synopsis="Classes dealing with lazy-rendered HTTP responses.">

django.template.response

</div>

Standard `~django.http.HttpResponse` objects are static structures. They are provided with a block of pre-rendered content at time of construction, and while that content can be modified, it isn't in a form that makes it easy to perform modifications.
...
The name of the template to be rendered. Accepts a backend-dependent template object (such as those returned by `~django.template.loader.get_template`), the name of a template, or a list of template names.
...
SimpleTemplateResponse.\_\_init\_\_(template, context=None, content_type=None, status=None, charset=None, using=None, headers=None)

Instantiates a `~django.template.response.SimpleTemplateResponse` object with the given template, context, content type, HTTP status, and charset.

`template`  
A backend-dependent template object (such as those returned by `~django.template.loader.get_template`), the name of a template, or a list of template names.
...
Resolves the template instance to use for rendering. Accepts a backend-dependent template object (such as those returned by `~django.template.loader.get_template`), the name of a template, or a list of template names.
...
If the `~django.template.response.SimpleTemplateResponse` has already been rendered, the callback will be invoked immediately.

When called, callbacks will be passed a single argument -- the rendered `~django.template.response.SimpleTemplateResponse` instance.
...
<div class="TemplateResponse()">

`TemplateResponse` is a subclass of `~django.template.response.SimpleTemplateResponse` that knows about the current `~django.http.HttpRequest`.
...
TemplateResponse.\_\_init\_\_(request, template, context=None, content_type=None, status=None, charset=None, using=None, headers=None)

Instantiates a `~django.template.response.TemplateResponse` object with the given request, template, context, content type, HTTP status, and charset.

`request`  
An `~django.http.HttpRequest` instance.
...
Before a `~django.template.response.TemplateResponse` instance can be returned to the client, it must be rendered. The rendering process takes the intermediate representation of template and context, and turns it into the final byte stream that can be served to the client.
...
A `TemplateResponse` object can be used anywhere that a normal `django.http.HttpResponse` can be used. It can also be used as an alternative to calling `~django.shortcuts.render`.
