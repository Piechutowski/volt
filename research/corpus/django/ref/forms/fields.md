...
<div class="module" synopsis="Django's built-in form fields.">
...
As explained in `ref-forms-api-outputting-html`, the default label for a `Field` is generated from the field name by converting all underscores to spaces and upper-casing the first letter. Specify a string for `label` if that default behavior doesn't result in an adequate label. Use an empty string (`""`) to hide the label.
...
The `label_suffix` argument lets you override the form's `~django.forms.Form.label_suffix` on a per-field basis:
...
The `widget` argument lets you specify a `Widget` class to use when rendering this `Field`. See `/ref/forms/widgets` for more information.
...
Like the model field's `~django.db.models.Field.help_text`, this value isn't HTML-escaped in automatically-generated forms.
...
When adding a custom `aria-describedby` attribute, make sure to also include the `id` of the `help_text` element (if used) in the desired order. For screen reader users, descriptions will be read in their order of appearance inside `aria-describedby`:
...
See the `validators documentation </ref/validators>` for more information.
...
See the `format localization documentation </topics/i18n/formatting>` for more information.
...
The `template_name` argument allows a custom template to be used when the field is rendered with `~django.forms.BoundField.as_field_group`. By default this value is set to `"django/forms/field.html"`. Can be changed per field by overriding this attribute or more generally by overriding the default template, see also `overriding-built-in-field-templates`.
...
<div class="CharField(**kwargs)">

- Default widget: `TextInput`
- Empty value: Whatever you've given as `empty_value`.
- Normalizes to: A string.
- Uses `~django.core.validators.MaxLengthValidator` and `~django.core.validators.MinLengthValidator` if `max_length` and `min_length` are provided. Otherwise, all inputs are valid.
...
Either an `iterable` of 2-tuples to use as choices for this field, `enumeration type <field-choices-enum-types>`, or a callable that returns such an iterable. This argument accepts the same formats as the `choices` argument to a model field. See the `model field reference documentation on choices <field-choices>` for more details. If the argument is a callable, it is evaluated each time the field's form is initialized, in addition to during rendering. Defaults to an empty list.
...
If no `input_formats` argument is provided, the default input formats are taken from the active locale format `DATE_INPUT_FORMATS` key, or from `DATE_INPUT_FORMATS` if localization is disabled. See also `format localization </topics/i18n/formatting>`.

</div>

### `DateTimeField`

<div class="DateTimeField(**kwargs)">
...
The field always accepts strings in ISO 8601 formatted dates or similar recognized by `~django.utils.dateparse.parse_datetime`. Some examples are:
...
If no `input_formats` argument is provided, the default input formats are taken from the active locale format `DATETIME_INPUT_FORMATS` and `DATE_INPUT_FORMATS` keys, or from `DATETIME_INPUT_FORMATS` and `DATE_INPUT_FORMATS` if localization is disabled. See also `format localization </topics/i18n/formatting>`.

</div>

### `DecimalField`

<div class="DecimalField(**kwargs)">

- Default widget: `NumberInput` when `Field.localize` is `False`, else `TextInput`.
- Empty value: `None`
- Normalizes to: A Python `decimal`.
- Validates that the given value is a decimal. Uses `~django.core.validators.MaxValueValidator` and `~django.core.validators.MinValueValidator` if `max_value` and `min_value` are provided. Uses `~django.core.validators.StepValueValidator` if `step_size` is provided. Leading and trailing whitespace is ignored.
...
Limit valid inputs to an integral multiple of `step_size`. If `min_value` is also provided, it's added as an offset to determine if the step size matches.
...
<div class="DurationField(**kwargs)">

- Default widget: `TextInput`
- Empty value: `None`
- Normalizes to: A Python `~python:datetime.timedelta`.
...
Accepts any format understood by `~django.utils.dateparse.parse_duration`.

</div>

### `EmailField`

<div class="EmailField(**kwargs)">

- Default widget: `EmailInput`
- Empty value: Whatever you've given as `empty_value`.
- Normalizes to: A string.
- Uses `~django.core.validators.EmailValidator` to validate that the given value is a valid email address, using a moderately complex regular expression.
- Error message keys: `required`, `invalid`

Has the optional arguments `max_length`, `min_length`, and `empty_value` which work just as they do for `CharField`. The `max_length` argument defaults to 320 (see `3696#section-3`).

</div>

### `FileField`

<div class="FileField(**kwargs)">
...
When you use a `FileField` in a form, you must also remember to `bind the file data to the form <binding-uploaded-files>`.
...
<div class="FilePathField(**kwargs)">
...
<div class="FloatField(**kwargs)">

- Default widget: `NumberInput` when `Field.localize` is `False`, else `TextInput`.
- Empty value: `None`
- Normalizes to: A Python float.
- Validates that the given value is a float. Uses `~django.core.validators.MaxValueValidator` and `~django.core.validators.MinValueValidator` if `max_value` and `min_value` are provided. Uses `~django.core.validators.StepValueValidator` if `step_size` is provided. Leading and trailing whitespace is allowed, as in Python's `float()` function.
...
<div class="GenericIPAddressField(**kwargs)">
...
Defaults to 39, and behaves the same way as it does for `CharField`.
...
<div class="ImageField(**kwargs)">

- Default widget: `ClearableFileInput`
- Empty value: `None`
- Normalizes to: An `UploadedFile` object that wraps the file content and file name into a single object.
- Validates that file data has been bound to the form. Also uses `~django.core.validators.FileExtensionValidator` to validate that the file extension is supported by Pillow.
...
When you use an `ImageField` on a form, you must also remember to `bind the file data to the form <binding-uploaded-files>`.
...
<div class="IntegerField(**kwargs)">

- Default widget: `NumberInput` when `Field.localize` is `False`, else `TextInput`.
- Empty value: `None`
- Normalizes to: A Python integer.
- Validates that the given value is an integer. Uses `~django.core.validators.MaxValueValidator` and `~django.core.validators.MinValueValidator` if `max_value` and `min_value` are provided. Uses `~django.core.validators.StepValueValidator` if `step_size` is provided. Leading and trailing whitespace is allowed, as in Python's `int()` function.
- Error message keys: `required`, `invalid`, `max_value`, `min_value`, `step_size`
...
<div class="JSONField(encoder=None, decoder=None, **kwargs)">

A field which accepts JSON encoded data for a `~django.db.models.JSONField`.
...
A `json.JSONEncoder` subclass to serialize data types not supported by the standard JSON serializer (e.g. `datetime.datetime` or `~python:uuid.UUID`). For example, you can use the `~django.core.serializers.json.DjangoJSONEncoder` class.
...
A `json.JSONDecoder` subclass to deserialize the input. Your deserialization may need to account for the fact that you can't be certain of the input type. For example, you run the risk of returning a `datetime` that was actually a string that just happened to be in the same format chosen for `datetime`s.
...
If you use a `ModelForm <django.forms.ModelForm>`, the `encoder` and `decoder` from `~django.db.models.JSONField` will be used.
...
<div class="MultipleChoiceField(**kwargs)">
...
<div class="NullBooleanField(**kwargs)">
...
`NullBooleanField` may be used with widgets such as `~django.forms.Select` or `~django.forms.RadioSelect` by providing the widget `choices`:
...
<div class="RegexField(**kwargs)">

- Default widget: `TextInput`
- Empty value: Whatever you've given as `empty_value`.
- Normalizes to: A string.
- Uses `~django.core.validators.RegexValidator` to validate that the given value matches a certain regular expression.
- Error message keys: `required`, `invalid`
...
<div class="SlugField(**kwargs)">

- Default widget: `TextInput`
- Empty value: Whatever you've given as `empty_value`.
- Normalizes to: A string.
- Uses `~django.core.validators.validate_slug` or `~django.core.validators.validate_unicode_slug` to validate that the given value contains only letters, numbers, underscores, and hyphens.
- Error messages: `required`, `invalid`

This field is intended for use in representing a model `~django.db.models.SlugField` in forms.
...
If no `input_formats` argument is provided, the default input formats are taken from the active locale format `TIME_INPUT_FORMATS` key, or from `TIME_INPUT_FORMATS` if localization is disabled. See also `format localization </topics/i18n/formatting>`.

</div>

### `TypedChoiceField`

<div class="TypedChoiceField(**kwargs)">
...
<div class="TypedMultipleChoiceField(**kwargs)">
...
<div class="URLField(**kwargs)">

- Default widget: `URLInput`
- Empty value: Whatever you've given as `empty_value`.
- Normalizes to: A string.
- Uses `~django.core.validators.URLValidator` to validate that the given value is a valid URL.
- Error message keys: `required`, `invalid`
...
<div class="UUIDField(**kwargs)">

- Default widget: `TextInput`
- Empty value: `None`
- Normalizes to: A `~python:uuid.UUID` object.
- Error message keys: `required`, `invalid`

This field will accept any string format accepted as the `hex` argument to the `~python:uuid.UUID` constructor.
...
<div class="MultiValueField(fields=(), **kwargs)">
...
This field is abstract and must be subclassed. In contrast with the single-value fields, subclasses of `MultiValueField` must not implement `~django.forms.Field.clean` but instead - implement `~MultiValueField.compress`.
...
A tuple of fields whose values are cleaned and subsequently combined into a single value. Each value of the field is cleaned by the corresponding field in `fields` -- the first value is cleaned by the first field, the second value is cleaned by the second field, etc. Once all fields are cleaned, the list of clean values is combined into a single value by `~MultiValueField.compress`.
...
Must be a subclass of `django.forms.MultiWidget`. Default value is `~django.forms.TextInput`, which probably is not very useful in this case.
...
<div class="SplitDateTimeField(**kwargs)">
...
<div class="ModelChoiceField(**kwargs)">
...
Note that no empty choice is created (regardless of the value of `empty_label`) if a `ModelChoiceField` is required and has a default initial value, or a `widget` is set to `~django.forms.RadioSelect` and the `~ModelChoiceField.blank` argument is `False`.
...
The default empty label was changed from `"---------"` to `"- Select an option -"`.
...
When using the `~django.forms.RadioSelect` widget, this optional boolean argument determines whether an empty choice is created. By default, `blank` is `False`, in which case no empty choice is created.
...
<div class="ModelMultipleChoiceField(**kwargs)">
...
You can use a `~django.forms.Select` widget subclass to include the value of `Topping.price` as the HTML attribute `data-price` for each `<option>` element:
...
<div class="ModelChoiceIterator(field)">
...
<div class="ModelChoiceIteratorValue(value, instance)">
...
You can also customize how a field will be accessed by overriding `~django.forms.Field.bound_field_class` or override `.Field.get_bound_field` if you need more flexibility when creating the `BoundField`:

<div class="method">

Field.get_bound_field(form, field_name)

Takes an instance of `~django.forms.Form` and the name of the field. The returned `.BoundField` instance will be used when accessing the field in a template.
