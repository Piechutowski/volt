...
This advanced tutorial begins where `Tutorial 8 </intro/tutorial08>` left off. We'll be turning our web-poll into a standalone Python package you can reuse in new projects and share with other people.
...
Let's say you were starting a new project that needed a polls app like the one we've been working on. How do you make this app reusable? Luckily, you're well on the way already. In `Tutorial 1 </intro/tutorial01>`, we saw how we could decouple polls from the project-level URLconf using an `include`. In this tutorial, we'll take further steps to make the app easy to use in new projects and ready to publish for others to install and use.
...
1.  First, create a parent directory for the package, outside of your Django project. Call this directory `django-polls`.
    ...
    Application labels (that is, the final part of the dotted path to application packages) *must* be unique in `INSTALLED_APPS`. Avoid using the same label as any of the Django `contrib packages
...
3.  Edit `django_polls/apps.py` so that `~.AppConfig.name` refers to the new module name, add `~.AppConfig.label` to give a short name for the app, and set `~.AppConfig.default_auto_field` to ensure your migrations are not affected by changes to `DEFAULT_AUTO_FIELD` made by users of your reusable app:
...
9.  Check that the `build` package is installed (`python -m pip install build`) and try building your package by running `python -m build` inside `django-polls`. This creates a directory called `dist` and builds your new package into source and binary formats, `django_polls-0.1.tar.gz` and `django_polls-0.1-py3-none-any.whl`.
...
1.  To install the package, use pip (you already `installed it
...
- Post the package on a public repository, such as [the Python Package Index (PyPI)](). There is [a good tutorial](https://packaging.python.org/tutorials/packaging-projects/#uploading-the-distribution-archives) for doing this.
...
Typically, these situations only arise once you're maintaining several Django projects. When they do, the best solution is to use `venv
<python:tutorial/venv>`. This tool allows you to maintain multiple isolated Python environments, each with its own copy of the libraries and package namespace.
