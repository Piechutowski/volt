...
| `edbml/ast`, `lang/parser` | syntax only — no semantic judgment, multi-error recovery |
| `lang/check` | semantics + `check.Info` symbol table (partials expanded, refs resolved) — **all generators consume Info, never re-derive from the AST** |
| `lang/vet` | analyzer framework + rules; docs in `edbml/vet/RULES.md` |
