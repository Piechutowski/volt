...
func (c *checker) resourcesExpand(res *ast.Resources, inh inherited) []*RouteInfo {
	...
	if res.Name.Quoted() || !goIdentOK(tableName) {
		c.errorf(res.Name.Pos(), "V5", "resources name %q must be a plain identifier (§V4.1.6)", tableName)
		return nil
	}
	controller, err := golang.GoName(tableName)
	if err != nil {
		c.errorf(res.Name.Pos(), "V5", "resources name %q: %v (§V5.1)", tableName, err)
		return nil
	}
...
// modelResolve resolves a [model:] setting to the referenced table's
// primary-key parameter type (§V5.4).
func (c *checker) modelResolve(s *ast.Setting) (ParamType, bool) {
	var pkgPath, modelBase string
	var pos token.Position
	switch v := s.Value.(type) {
	case *ast.Ident:
		pkgPath, modelBase, pos = c.pkg.Path, v.Name(), v.Pos()
	case *ast.EnumConst:
		qual := v.Enum.Name()
		target, ok := c.pkg.Imports[qual]
		if !ok {
			c.errorf(v.Pos(), "V5", "unknown package qualifier %q in model reference (§V5.4)", qual)
			return "", false
		}
		c.usedQual[qual] = true
		pkgPath, modelBase, pos = target, v.Value.Name(), v.Pos()
	default:
		c.errorf(s.Pos(), "V5", "model: takes Model or pkg.Model (§V5.4)")
		return "", false
	}
	...
	for _, ti := range info.Tables {
		name, err := golang.ModelName(ti.Decl)
		if err != nil || name != modelBase {
			continue
		}
		return c.pkParamType(ti, pos)
	}
	c.errorf(pos, "V5", "no table in package %q has model name %q (§V5.4)", pkgPath, modelBase)
	return "", false
}
